let { account, savings, cash, savings_goal, track_gym, lang_opt } = input;
...
class DataRepository {
  ...
  fetch(ref, scope) {
    ...
    const journal = pages
      .where(
        (p) => p.file.folder && p.file.folder.includes("journal") && p.file.day,
      )
      .where((p) =>
        scope === "week"
...
async function indexData(
  ...
  if (finance.byMonth[DateUtils.monthIndexer(ref)]) {
